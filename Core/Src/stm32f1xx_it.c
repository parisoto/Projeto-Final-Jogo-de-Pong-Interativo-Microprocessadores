...
#include "keypad.h" // Inclui o keypad.h para a função KEYPAD_ReadKey e variáveis
#include "my_font.h"
#include "st7789.h"
...
/* USER CODE BEGIN EV */
extern volatile uint8_t g_keypad_active;
extern volatile char g_keypad_buffer[16];
extern volatile uint8_t g_buffer_index;
// Adicione também a declaração de qualquer função externa de desenho que você use no IRQ
extern void ST7789_DrawCharScaled(uint16_t x, uint16_t y, char character, uint16_t fg_color, uint16_t bg_color, uint8_t scale);
/* USER CODE END EV */
